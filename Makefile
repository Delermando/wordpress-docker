up:
	docker run -d  --name="mysql55-wordpress1"  -e MYSQL_ROOT_PASSWORD=root -p 3306:3306    -v `pwd`/mysql-data/var/lib/mysql:/var/lib/mysql:rw  mysql:5.5;
	docker run -d   --privileged=true --name="wordpress1" --link mysql55-wordpress1:mysql  -p 80:80 -v `pwd`/vhosts:/etc/apache2/vhosts:rw    -v `pwd`/wordpress:/var/www/html:rw rtancman/php:php53-apache22;
